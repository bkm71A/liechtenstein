...
/**
 * Butterworth Digital Filter Implementation - High Pass Filter. This is a Butterworth algorithm implementation that
 * allows to define filter Forder.
 * <p>
 ...
 *         Change History:
 *         <ul>
 *         <li>05/30/2008 Taras Dobrovolsky Initial release
 *         <li>06/09/2008 Taras Dobrovolsky Fixed indexing problem
 *         </ul>
 */
public class ButterHighPassFilterOrd implements Filter {
    ...
    /**
     * This method performs high-pass (HP) digital filtering using Butterworth method with specified filter order.
    ...
    public List<Double> filter( List<Double> y, int s, double f, int st ) throws InvalidAlgorithmParameterException {
        ...
        if( (y == null) || y.size() < 2 ) {
            throw new InvalidAlgorithmParameterException(
                                                          "com.incheck.dpm.dsp.ButterHighPassFilterOrd: Invalid input array of zero length or Null.\n\t " );
        }
        ...
        if( (st % 2 != 0) || (st < 2) ) {
            throw new InvalidAlgorithmParameterException(
                                                          "com.incheck.dpm.dsp.ButterHighPassFilterOrd: Invalid filter order parameter.\n\t " );
        }
        ...
        int h = y.size();
        double vt[] = new double[h + 2]; // temporary array used in calculation for each stage
        double v[] = new double[h + 2]; // temporary arrays that have 2 more members than input array
        double x[] = new double[h + 2];
        ...
        for( i = 0; i < j; i++ ) {
            x[0] = x[h]; // assign x[0], x[1] from the back of the array
            x[1] = x[h + 1];

            for( k = 2; k < x.length; k++ ) { // calculate the temporary array
                vt[k] = (b0[i] * x[k] + b1[i] * x[k - 1] + b2[i] * x[k - 2]) - (a1[i] * vt[k - 1] + a2[i] * vt[k - 2]);
            }

            v[0] = vt[h]; // assign v[0], v[1] from the temporary array
            v[1] = vt[h + 1];
