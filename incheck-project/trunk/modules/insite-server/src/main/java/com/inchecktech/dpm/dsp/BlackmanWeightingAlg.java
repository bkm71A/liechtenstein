...
public class BlackmanWeightingAlg implements WeightingAlg {
	...
	/**
	 * Blackman weighting or windowing method.  Weighting performed in place without
	...
	public void weight(List<Double> arrInOut) {
		...
		for (Double number : arrInOut) {
			double multiplier = 0.42 - 0.5 * Math.cos(2 * Math.PI * n / N) + 0.08
					* Math.cos(4 * Math.PI * n / N);
