...
/**
 * Butterworth Digital Filter Implementation - Low Pass Filter This is a 6-th order Butterworth algorithm implementation
 * using method documented in "Shock and Vibration Signal Analysis" by Tom Irvine. pp. 82-85
 * @author Taras Dobrovolsky Change History:n 12/22/2007 Taras Dobrovolsky Initial release
 */
public class ButterLowPassFilter implements Filter {
     ...
     * @param optParam1 optional filter parameter. Not used in this implementation of the filter.
     * @return digitally filtered array
     */
    public List<Double> filter( List<Double> arrIn, int samplingRate, double cutoffFreq, int optParam1 )
                                                                                                        throws InvalidAlgorithmParameterException {
        return filter( arrIn, samplingRate, cutoffFreq );
    }
    ...
    public List<Double> filter( List<Double> x, int samplingRate, double cutoffFreq )
        ...
        if( (x == null) || x.size() == 0 ) {
            throw new InvalidAlgorithmParameterException(
                                                          "com.incheck.dpm.dsp.ButterLowPassFilter: Invalid input array of zero length or Null.\n\t " );
        }
        if( cutoffFreq <= 0 ) {
            return x;
        }
        ...
        double a11 = 2 * (Qc * Qc - 1) / (Qc * Qc - alpha1 * Qc + 1);
        double a12 = (1 + alpha1 * Qc + Qc * Qc) / (Qc * Qc - alpha1 * Qc + 1);
        double b10 = Qc * Qc / (Qc * Qc - alpha1 * Qc + 1);
        double b11 = 2 * Qc * Qc / (Qc * Qc - alpha1 * Qc + 1);
        double b12 = b10;

        // STAGE 2 coefficients
        double a21 = 2 * (Qc * Qc - 1) / (Qc * Qc - alpha2 * Qc + 1);
        double a22 = (1 + alpha2 * Qc + Qc * Qc) / (Qc * Qc - alpha2 * Qc + 1);
        double b20 = Qc * Qc / (Qc * Qc - alpha2 * Qc + 1);
        double b21 = 2 * Qc * Qc / (Qc * Qc - alpha2 * Qc + 1);
        double b22 = b20;

        // STAGE 3 coefficients
        double a31 = 2 * (Qc * Qc - 1) / (Qc * Qc - alpha3 * Qc + 1);
        double a32 = (1 + alpha3 * Qc + Qc * Qc) / (Qc * Qc - alpha3 * Qc + 1);
        double b30 = Qc * Qc / (Qc * Qc - alpha3 * Qc + 1);
        double b31 = 2 * Qc * Qc / (Qc * Qc - alpha3 * Qc + 1);
        double b32 = b30;
        ...
        // STAGE #2
        for( i = 2; i < k; i++ ) {
            y2.set( i, b20 * y1.elementAt( i ) + b21 * y1.elementAt( i - 1 ) + b22 * y1.elementAt( i - 2 )
                       - ((a21 * y2.elementAt( i - 1 ) + a22 * y2.elementAt( i - 2 ))) );
        }

        // STAGE #3
        for( i = 2; i < k; i++ ) {
            y3.set( i, b30 * y2.elementAt( i ) + b31 * y2.elementAt( i - 1 ) + b32 * y2.elementAt( i - 2 )
                       - ((a31 * y3.elementAt( i - 1 ) + a32 * y3.elementAt( i - 2 ))) );
        }
