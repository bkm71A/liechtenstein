package com.incheck.charts
{
	...
	public class RangeSelectorAnalysis extends ChartElement
	{
		...
		public function RangeSelectorAnalysis():void
		{
			...
			//FlexGlobals.topLevelApplication.addEventListener(MouseEvent.MOUSE_MOVE,debugMove, false, 0, true);
//            FlexGlobals.topLevelApplication.addEventListener(KeyboardEvent.KEY_DOWN, keyPressed, false, 0, true);
//			FlexGlobals.topLevelApplication.addEventListener(KeyboardEvent.KEY_DOWN, keyDePressed, false, 0, true);
			addEventListener(MouseEvent.ROLL_OVER, mouseRollOver, false, 0, true);

			BindingUtils.bindSetter(cursorTypeChanged, IncheckModel.instance(), "cursorMenuSelectionAnalysis");
			BindingUtils.bindSetter(functionTypeChanged, IncheckModel.instance(), "functionMenuSelectionAnalysis");
			BindingUtils.bindSetter(chartTypeChanged, IncheckModel.instance(), "graphMenuSelectionAnalysis");

			lastKnownGraph = IncheckModel.instance().graphMenuSelectionAnalysis;
			lastKnownFunction = IncheckModel.instance().functionMenuSelectionAnalysis;
			lastKnownCursor = IncheckModel.instance().cursorMenuSelectionAnalysis;
		}
		...
		*/
		...
		private function mouseDown(e:MouseEvent):void
		{
			...
			mouseIsDown = true;
			var dataVals:Array;
			if (IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.Single || IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.Double || IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.Harmonic || IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.Sideband)
			{
			...
			if (IncheckModel.instance().scalingMenuSelectionAnalysis == DataViewScalingTypes.HZoom || e.ctrlKey)
			{
				dragStartPoint = new Point(mouseX, mouseY);
			...
			else if (IncheckModel.instance().scalingMenuSelectionAnalysis == DataViewScalingTypes.VZoom || e.shiftKey)
			{
				dragStartPoint = new Point(mouseX, mouseY);
				dataVals = dataTransform.invertTransform(mouseX, mouseY);
				tX = dataVals[0];
			...
			else if (IncheckModel.instance().scalingMenuSelectionAnalysis == DataViewScalingTypes.Pan || e.altKey)
			{
				dragStartPoint = new Point(mouseX, mouseY);
				dataVals = dataTransform.invertTransform(mouseX, mouseY);
				tX = dataVals[0];
		...
		{
			if (IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.Single || IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.Sideband || (IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.Harmonic && IncheckModel.instance().functionMenuSelectionAnalysis != "wave"))
			{
				nudgingCursorOne = true;
				_cursorOne = p;
			}
			else if (IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.Double || (IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.Harmonic && IncheckModel.instance().functionMenuSelectionAnalysis == "wave"))
			{
				if (_cursorOne == null)
				{
					_cursorOne = p;
		...
		private function updateTrackBounds(dataVals:Array):void
		{
			...
			if (zoomType == "h")
			{
				...
				if (areWeZooming == false)
				{
					...
					(IncheckModel.instance().right.analysisChart.horizontalAxis as LinearAxis).maximum = dRight;
					(IncheckModel.instance().right.analysisChart.horizontalAxis as LinearAxis).minimum = dLeft;
					dRight = 0;
					dLeft = 0;
					dBottom = 0;
					dTop = 0;
...
//              var ratio:Number = newMin / _startMin.y;
				...
				/*
					var curTop:Number=(IncheckModel.instance().right.dataViewChart.verticalAxis as LinearAxis).maximum;
					var curBot:Number=(IncheckModel.instance().right.dataViewChart.verticalAxis as LinearAxis).minimum;
					...
					if(distanceMoved>0){
						...
						(IncheckModel.instance().right.dataViewChart.verticalAxis as LinearAxis).maximum=curTop+(currRangeShown/distanceMoved);
			...
			else if (zoomType == "p")
			{
				...
				if (proposedHMax > IncheckModel.instance().right.analysisChart.xmax || proposedHMin < IncheckModel.instance().right.analysisChart.xmin)
				{
					return;
				}
				else
				...
				if (proposedVMax > IncheckModel.instance().right.analysisChart.ymax || proposedVMin < IncheckModel.instance().right.analysisChart.ymin)
				{
		...
		private function mouseUp(e:MouseEvent):void
		{
			...
			{
				if ((IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.Double || IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.Harmonic && IncheckModel.instance().functionMenuSelectionAnalysis == "wave") && _cursorOne != null && _cursorTwo != null)
				{
		...
		override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
		{
			...
			xTit = ((chart as IncheckChart3).horizontalAxis as LinearAxis).title
			yTit = ((chart as IncheckChart3).verticalAxis as LinearAxis).title
			...
			if (IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.Harmonic && _cursorOne != null)
			{
				if (IncheckModel.instance().functionMenuSelectionAnalysis == "wave")
				{
					...
					if (_cursorTwo != null)
					{
						...
						while (movingCursorPosition < unscaledWidth && hStepCntr < IncheckModel.instance().right.analysisMenuBar.numOfExtraCursors.value + 1)
						{
...
//								}
				...
				else
				{
					...
					hStepCntr = 2;
					prevIndex = 0;
					while (currentPos < unscaledWidth && hStepCntr < IncheckModel.instance().right.analysisMenuBar.numOfExtraCursors.value + 1)
					{
						g.lineStyle(2, 0x005364, .5);
...
//							}
			...
			else if (IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.Sideband && _cursorOne != null)
			{
				...
				//Alert.show("HERE" + movingCursorPosition+ " "+ stepSize +" "+unscaledWidth +" "+_cursorTwo.x +" "+_cursorOne.x);
				while (hStepCntr * 2 < IncheckModel.instance().right.analysisMenuBar.numOfExtraCursors.value + 1)
				{
...
//						lab2.text = "\n" + hStepCntr + "\n" + toScientific(hitObj.y) + " " + yTit + "\n" + toScientific(hitObj.x) + " " + xTit;
			...
			else if (IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.PeakFinder)
			{
				mydp = mySeries.dataProvider as ArrayCollection;
				...
				while (peaksFound < IncheckModel.instance().right.analysisMenuBar.numOfExtraCursors.value)
				{
		...
		private function updateCursorOne(unscaledWidth:Number, unscaledHeight:Number, mySeries:InCheckLineSeries, shouldBold:Boolean):void
		{
			...
			if (hitObj != null)
			{
				...
				if (IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.Harmonic || IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.Double)
				{
//					lab2.text = "\n1\n" + toScientific(hitObj.y) + " " + yTit + "\n" + toScientific(hitObj.x) + " " + xTit;
					dataGridProvider.addItem({num:"1", y:toScientific(hitObj.y), x:toScientific(hitObj.x)});
					lab2.text = "\n1";
				}
				else if (IncheckModel.instance().cursorMenuSelectionAnalysis == DataViewCursorTypes.Single)
				{
					lab2.text = "\n" + toScientific(hitObj.y) + " " + yTit + "\n" + toScientific(hitObj.x) + " " + xTit;
				}
				else
	 ...
	 draw the selected region, if there is one
			...
			private function panAndZoom(e:MouseEvent):void
			{
				...
				if(_dragTool == "pan")
				{
					...
					var proposedHMax:Number=_startMax.x - dX * _start1PixelOffset[0];
					var proposedHMin:Number=_startMin.x - dX * _start1PixelOffset[0];
					if(proposedHMax>IncheckModel.instance().right.dataViewChart.xmax){
						hAxis.maximum =IncheckModel.instance().right.dataViewChart.xmax
						return;
					}

					if(proposedHMin<IncheckModel.instance().right.dataViewChart.xmin){
						proposedHMin=IncheckModel.instance().right.dataViewChart.xmin
						hAxis.minimum = proposedHMin
						return;
					}
					hAxis.maximum = proposedHMax;
	...
	//            Alert.show(pare);
			...
			private function updateTrackBounds(dataVals:Array):void
			{
				...
				if(bTracking==false){
					//(IncheckModel.instance().right.dataViewChart.verticalAxis as LinearAxis).maximum=dTop;
					//(IncheckModel.instance().right.dataViewChart.verticalAxis as LinearAxis).minimum=dBottom;
					(IncheckModel.instance().right.dataViewChart.horizontalAxis as LinearAxis).maximum=dRight;
					(IncheckModel.instance().right.dataViewChart.horizontalAxis as LinearAxis).minimum=dLeft;
					dRight=0
					dLeft=0;
					dBottom=0;
					dTop=0
