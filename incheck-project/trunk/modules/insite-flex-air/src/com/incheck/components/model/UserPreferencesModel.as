...
package com.incheck.components.model
{
	...
	{				
		...
		private var _numberOfAverages:int = 1;
		...
		[Bindable(event="numberOfAveragesChange")]
		public function get numberOfAverages():int
		{
			if (_numberOfAverages < 0) 
			{
				return 1;
			} 
			else
		...
		private function getAvailableUserPreferences():void
		{
			...
			ro.addEventListener(ResultEvent.RESULT, onGetAvailableUserPreferencesResult, false, 0, true);
			ro.addEventListener(FaultEvent.FAULT, onGetAvailableUserPreferencesFault, false, 0, true);
		...
		private function getUserPreferences(userId:int):void
		{
			...
			ro.addEventListener(ResultEvent.RESULT, onGetUserPreferencesResult, false, 0, true);
			ro.addEventListener(FaultEvent.FAULT, onGetUserPreferencesFault, false, 0, true);
		...
		private function unsetUserPreference(userId:int, userPreferenceId:int):void
		{
			...
			ro.addEventListener(ResultEvent.RESULT, onUnsetUserPreferenceResult, false, 0, true);
			ro.addEventListener(FaultEvent.FAULT, onUnsetUserPreferenceFault, false, 0, true);
		...
		private function setUserPreference(userId:int, userPreferenceId:int):void
		{
			...
			ro.addEventListener(ResultEvent.RESULT, onSetUserPreferenceResult, false, 0, true);
			ro.addEventListener(FaultEvent.FAULT, onSetUserPreferenceFault, false, 0, true);
		...
		public function updateNumberOfAverages(value:int):void
		{
			_numberOfAverages = value;
			dispatchEvent(new Event("numberOfAveragesChange"));
			enableSaveUserPreferences();
		}
