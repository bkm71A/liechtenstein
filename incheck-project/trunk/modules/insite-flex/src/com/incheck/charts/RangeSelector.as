package com.incheck.charts
{
    ...
    public class RangeSelector extends ChartElement
    {
       ...
       // private var _dragTool:String;
		...
		[Embed(source="/resources/hand.png")]
        [Bindable]
    	...
    	public function RangeSelector():void
    	{
			...
			FlexGlobals.topLevelApplication.addEventListener(MouseEvent.MOUSE_UP, mouseUp);
			...
			lastKnownGraph = IncheckModel.instance().graphMenuSelection;
			lastKnownFunction = IncheckModel.instance().functionMenuSelection;
			lastKnownCursor = IncheckModel.instance().cursorMenuSelection;
    	}
       ...
       public function keyDePressed(ev:KeyboardEvent):void
       {
    	...
    	private function mouseRollOver(e:MouseEvent):void
    	{
    		...
    		if (chart != null && !mouseMoveEventAdded)
    		{
    			...
    			chart.addEventListener(MouseEvent.MOUSE_MOVE, mouseMove);
    		}
    	}
   		
    	private function startZoom(e:MouseEvent):void
    	{
       		var hAxis:CustomLinearAxis = CustomLinearAxis(CartesianTransform(dataTransform).getAxis(CartesianTransform.HORIZONTAL_AXIS));
            var vAxis:CustomLinearAxis = CustomLinearAxis(CartesianTransform(dataTransform).getAxis(CartesianTransform.VERTICAL_AXIS));
    	...
    	private function mouseDown(e:MouseEvent):void
    	{
    		...
    		mouseIsDown = true;
    		var dataVals:Array;
    		if (IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.Single || IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.Double || IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.Harmonic ||	IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.Sideband)
    		{
    		...
    		if (IncheckModel.instance().scalingMenuSelection == DataViewScalingTypes.HZoom || e.ctrlKey )
    		{
    			dragStartPoint = new Point(mouseX, mouseY);
    		...
    		else if (IncheckModel.instance().scalingMenuSelection == DataViewScalingTypes.VZoom || e.shiftKey )
    		{
    			dragStartPoint = new Point(mouseX, mouseY);
    			dataVals = dataTransform.invertTransform(mouseX, mouseY);
	            tX = dataVals[0];
    		...
    		else if (IncheckModel.instance().scalingMenuSelection == DataViewScalingTypes.Pan || e.altKey)
    		{
    			dragStartPoint = new Point(mouseX, mouseY);
    			dataVals = dataTransform.invertTransform(mouseX, mouseY);
	            tX = dataVals[0];
    	...
    	{
    		if (IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.Single || IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.Sideband ||	(IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.Harmonic &&	IncheckModel.instance().functionMenuSelection != "wave"))
    		{
				nudgingCursorOne = true;
	    		_cursorOne = p;
	    	}
	    	else if (IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.Double || (IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.Harmonic && IncheckModel.instance().functionMenuSelection == "wave"))
	    	{
				if (_cursorOne == null)
				{
         			_cursorOne = p;
        ...
        {
        	var hAxis:CustomLinearAxis;
        	var vAxis:CustomLinearAxis;
        	if (zoomType == "h")
        	{
	            ...
	            if (areWeZooming == false)
	            {
	            	...
	            	(IncheckModel.instance().right.dataViewChart.horizontalAxis as CustomLinearAxis).maximum = dRight;
	            	(IncheckModel.instance().right.dataViewChart.horizontalAxis as CustomLinearAxis).minimum = dLeft;
					dRight = 0;
					dLeft = 0;
					dBottom = 0;
					dTop = 0;
	        ...
	        {
	        	hAxis = CustomLinearAxis(CartesianTransform(dataTransform).getAxis(CartesianTransform.HORIZONTAL_AXIS));
    	        vAxis = CustomLinearAxis(CartesianTransform(dataTransform).getAxis(CartesianTransform.VERTICAL_AXIS));
...
//              var ratio:Number = newMin / _startMin.y;
	        ...
	        else if (zoomType == "p")
	        {
				...
				if (proposedHMax > IncheckModel.instance().right.dataViewChart.xmax || proposedHMin < IncheckModel.instance().right.dataViewChart.xmin)
				{
					return;
				}
				else
				...
				if (proposedVMax > IncheckModel.instance().right.dataViewChart.ymax || proposedVMin < IncheckModel.instance().right.dataViewChart.ymin)
				{
    	...
    	private function mouseUp(e:MouseEvent):void
    	{
    		...
    		{
    			if ((IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.Double 
					|| IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.Harmonic && IncheckModel.instance().functionMenuSelection == "wave") 
					&& _cursorOne != null && _cursorTwo != null)
    			{
		...
		override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
	    {
			...
			xTit = ((chart as IncheckChart3).horizontalAxis as CustomLinearAxis).title
			yTit = ((chart as IncheckChart3).verticalAxis as CustomLinearAxis).title
			...
			if (IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.Harmonic && _cursorOne != null)
			{
				if (IncheckModel.instance().functionMenuSelection == "wave")
				{
					...
					if (_cursorTwo != null)
					{
						...
						while (movingCursorPosition < unscaledWidth &&  hStepCntr < IncheckModel.instance().right.dataViewMenuBar.numOfExtraCursors.value + 1)
						{
							g.lineStyle(2, 0x005364, .5);
							g.moveTo(movingCursorPosition, 0);
							g.lineTo(movingCursorPosition, unscaledHeight);
							lab2 = new UITextField();
							lab2.addEventListener(MouseEvent.MOUSE_DOWN, mouseDown);
            				lab2.addEventListener(MouseEvent.MOUSE_UP, mouseUp);
            				lab2.addEventListener(MouseEvent.MOUSE_MOVE, mouseMove);
...
//								}
				...
				else
				{
					...
					hStepCntr = 2;
					prevIndex = 0;
					while (currentPos<unscaledWidth && hStepCntr < IncheckModel.instance().right.dataViewMenuBar.numOfExtraCursors.value + 1)
					{
						g.lineStyle(2, 0x005364, .5);
...
//							}
			...
			else if (IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.Sideband && _cursorOne != null)
			{
				...
				//Alert.show("HERE" + movingCursorPosition+ " "+ stepSize +" "+unscaledWidth +" "+_cursorTwo.x +" "+_cursorOne.x);
				while (hStepCntr * 2 < IncheckModel.instance().right.dataViewMenuBar.numOfExtraCursors.value + 1)
				{
...
//						lab2.text = "\n" + hStepCntr + "\n" + toScientific(hitObj.y) + " " + yTit + "\n" + toScientific(hitObj.x) + " " + xTit;
			...
			else if (IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.PeakFinder)
			{
				mydp = mySeries.dataProvider as ArrayCollection;
				...
				while (peaksFound < IncheckModel.instance().right.dataViewMenuBar.numOfExtraCursors.value)
				{
		...
		private function updateCursorOne(unscaledWidth:Number, unscaledHeight:Number, mySeries:InCheckLineSeries, shouldBold:Boolean):void
		{
			...
			if (hitObj != null)
			{
				...
				if (IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.Harmonic || 
					IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.Double)
				{
//					lab2.text = "\n1\n" + toScientific(hitObj.y) + " " + yTit + "\n" + toScientific(hitObj.x) + " " + xTit;
					dataGridProvider.addItem({num:"1", y: toScientific(hitObj.y), x:toScientific(hitObj.x)});
					lab2.text = "\n1";
				}
				else if (IncheckModel.instance().cursorMenuSelection == DataViewCursorTypes.Single)
				{
					lab2.text = "\n" + toScientific(hitObj.y) + " " + yTit + "\n" + toScientific(hitObj.x) + " " + xTit;
				} 
				else 
...
//            		lab2.text= "\nΔ" + toScientific((_cursorTwoObj.x - _cursorOneObj.x)) + " " + yTit + "\n" + toScientific(_cursorTwoObj.x) + " " + xTit;
					...
					dataGridProvider.addItem({num:"Δ", x: toScientific(_cursorTwoObj.x - _cursorOneObj.x), y: toScientific(_cursorTwoObj.y - _cursorOneObj.y)});
            	}
            	else
            	{
