package com.incheck.admin
{
	...
	public class BandSettingsBase extends Canvas
	{
		...
		protected static const presetBands : Array = [
			{cooef : 1, name : "Overall"},
			{cooef : 2, name : "1x"},
			{cooef : 3, name : "2x"},
			{cooef : 4, name : "3x"},
			{cooef : 5, name : "Sub-1x"},
			{cooef : 6, name : "4x-9x"},
			{cooef : 7, name : "10x-30x"},
			{cooef : 8, name : "30x-60x"},
			{cooef : 9, name : "Custom"}
		];
		
		protected static const presetBandsForShow : Array = [
			{cooef : 2, name : "1x"},
			{cooef : 3, name : "2x"},
			{cooef : 4, name : "3x"},
			{cooef : 5, name : "Sub-1x"},
		...
		private function selectedNodeChanged(node:ICNode):void {
			if ((IncheckModel.instance().right as RightNavigator).settingsNav != null && IncheckModel.instance().right.settingsNav.selectedIndex == 0) {
				if (null != node) {
		...
		protected function tryCalculateFrequencies(index : uint, speed : String, constant : String) : void {
			if (speed && constant) {
				calculateFrequencies(presetBands[index].cooef, Number(speed), Number(constant));
				enableSave = !bandIsUsed(presetBands[index].cooef) && startFr && endFr;
			} else {
				startFr = "";
				endFr = "";
				enableSave = false;
		...
		private function bandIsUsed(presetBandId : uint) : Boolean {
			for each (var band : Band in bands) {
				if (band.band && band.band.cooef == presetBandId) {
					return true;
				}
			}
			return false;
		...
		private function calculateFrequencies(cooef : uint, speed : Number, constant : Number) : void {
			if (cooef == 1) {
				startFr = "0";
				endFr = overallEndFrequency.toString();
			} else {
				var result : Array = BandFrequencyCalculation.calculateFrequency(cooef, speed, constant);
				startFr = numberFormatter.format(result[0]);
				endFr = numberFormatter.format(result[1]);
			}
		}
			
		protected function addBand(bandName:String, bandCooef:String, speed : String, constant : String):void {
...
//			clearForm();
			...
			ro.createBandForChannel(bandName, bandCooef, Number(startFr), Number(endFr), selectedNode.channelid);
