package com.epam.bdru2016.team2.mss.processing.fbenrichment.facebook
...
class FbFacade(
                token:String,
                onSuccess:String=>Unit = s => println(s"FB call success: $s"),
                onError:String=>Unit = s => println(s"FB call error: $s")
              ) extends Logging {
  ...
  def getPlaces(keywords: Array[String], location:(Float,Float), distance:Int): Map[String,Array[Long]] = {
  ...
  private def executeWithRetry[A](batches:Iterable[BatchRequestWithPayload[A]]) : Array[BatchResponseWithPayload[A]] = {
    ...
    batches.grouped(50).flatMap(b => {
          ...
          if (result.exists(x => x.batchResponse.contains("Too many place searches"))) {
            onError(b.size.toString)
...
case class FacebookSettings(token:String, wordsLimit:Option[Int] = None)
